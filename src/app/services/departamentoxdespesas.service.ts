...
@Injectable()
export class DepartamentosXDespesasService extends Service{

    constructor(private http: Http) {
        super();
    }

    public getListDesenvolvimento(): Observable<any>{
        return this.http.get(
            ('assets/desenvolvimentodb/departamentoxdespesas.json') )
    ...
    public getListEnergia(): Observable<any>{
        return this.http.get(
            ('assets/energiadb/departamentoxdespesasenergia.json') )
        .pipe(retry(10), //Permite que possamos colocar o número de tentativas
              map((resposta:Response) => resposta.json())) // Conseguimos assim transformar o retorno para um objeto - Somente o conteudo da resposta
              //Utilizamos o MAP para transformar no objeto esperado.
    }
